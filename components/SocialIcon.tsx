...
export const XTwitterIcon: React.FC<{ size?: number; className?: string }> = ({ size = 20, className = "" }) => (
...
export const SocialIcon: React.FC<SocialIconProps> = ({ iconName, className = "w-5 h-5", size = 20 }) => {
  ...
  if (normalized.includes('mirror') || normalized.includes('blog') || normalized.includes('substack') || normalized.includes('medium')) {
    return <FileText size={size} className={className} />;
  }

  if (normalized.includes('farcaster') || normalized.includes('warpcast')) {
...
export const AVAILABLE_SOCIAL_ICONS = [
  ...
  { label: 'Mirror / Blog', value: 'Mirror' },
  { label: 'Warpcast / Farcaster', value: 'Warpcast' },
  { label: 'Email', value: 'Mail' },
  { label: 'Website / Other', value: 'Globe' },
];
